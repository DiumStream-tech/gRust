...
function gRust.SwapInventorySlots(pl, inventory1, index1, inventory2, index2, quantity)
    ...
    canAccess, reason = inventory2:CanPlayerAccess(pl)
    if (not canAccess) then
        pl:ChatPrint(reason)
        return
    end
...
net.Receive("gRust.SwapInventorySlots", function(len, pl)
    ...
    if (invIndex1 == invIndex2 and index1 == index2) then return end

    local inventory1 = gRust.Inventories[invIndex1]
    local inventory2 = gRust.Inventories[invIndex2]
    if (not inventory1 or not inventory2) then return end

    if (not inventory1[index1] and not inventory2[index2]) then return end

    local sourceInventory, sourceIndex, destInventory, destIndex

    if (inventory1[index1]) then
        sourceInventory = inventory1
        sourceIndex     = index1
        destInventory   = inventory2
        destIndex       = index2
    else
        sourceInventory = inventory2
        sourceIndex     = index2
        destInventory   = inventory1
        destIndex       = index1
    end

    local register = sourceInventory[sourceIndex]:GetRegister()
...
net.Receive("gRust.DropItem", function(len, pl)
    ...
    if (not inventory:CanPlayerAccess(pl)) then return end

    if (not inventory[index]) then return end
