...
local FavouriteItems = {}

local function LoadFavourites()
    FavouriteItems = {}
    local raw = cookie.GetString("gRust_Favourites", "")
    if (raw and raw ~= "") then
        for itemId in string.gmatch(raw, "([^,]+)") do
            FavouriteItems[itemId] = true
        end
    end
end
...
local function SaveFavourites()
    local items = {}
    for itemId, _ in pairs(FavouriteItems) do
        table.insert(items, itemId)
    end
    cookie.Set("gRust_Favourites", table.concat(items, ","))
end

local function IsFavourite(itemId)
    return FavouriteItems[itemId] == true
end

local function ToggleFavourite(itemId)
    if (IsFavourite(itemId)) then
        FavouriteItems[itemId] = nil
    else
        FavouriteItems[itemId] = true
    end
    SaveFavourites()

    -- Mise à jour de la catégorie Favourite
    local favCat = gRust.GetCategory("Favourite")
    if (favCat) then
        favCat.Items = {}
        for id, _ in pairs(FavouriteItems) do
            table.insert(favCat.Items, id)
        end
    end
end
...
-- ============================================================
-- [FIX] Synchronisation de la file de craft côté client.
-- Ces net.Receive étaient entièrement absents : pl.CraftQueue
-- n'était jamais rempli, et gRust.Craft / gRust.GetCraftingAmount
-- n'existaient pas → erreur Lua dans craftgrid_cl.lua:123.
-- ============================================================

-- Ajout d'un item dans la file (serveur → client)
net.Receive("gRust.AddToCraftQueue", function()
    ...
    hook.Run("gRust.CraftQueueUpdated")
end)

-- Pop d'un item crafté (serveur → client)
net.Receive("gRust.PopCraftingQueue", function()
    local pl = LocalPlayer()
    if (not pl.CraftQueue or #pl.CraftQueue == 0) then return end

    local item = pl.CraftQueue[1]
...
-- Retrait d'un item de la file (serveur → client)
net.Receive("gRust.RemoveFromCraftingQueue", function()
    local pl = LocalPlayer()
    if (not pl.CraftQueue) then return end
...
-- Clear complet de la file (serveur → client)
net.Receive("gRust.ClearCraftingQueue", function()
    local pl = LocalPlayer()
    pl.CraftQueue = {}
    hook.Run("gRust.CraftQueueUpdated")
end)

-- Met à jour le timer de fin du premier item dans la file (pour l'affichage)
hook.Add("Think", "gRust.UpdateCraftQueueTimer", function()
    local pl = LocalPlayer()
    if (not IsValid(pl) or not pl.CraftQueue or #pl.CraftQueue == 0) then return end

    local item = pl.CraftQueue[1]
    if (not item or item.End) then return end

    local register = gRust.GetItemRegister(item.Item)
    if (not register) then return end

    item.End = CurTime() + register:GetCraftTime()
end)

-- Envoie une demande de craft au serveur (client → serveur)
function gRust.Craft(id, amount)
    ...
    net.Start("gRust.Craft")
        ...
        net.WriteUInt(math.Clamp(amount or 1, 1, 255), 8)
    net.SendToServer()
end

-- Demande au serveur de retirer un item de la file (client → serveur)
...
-- Retourne la quantité totale en attente pour un item dans la file.
-- Appelée par craftgrid_cl.lua pour afficher le badge sur l'icône.
function gRust.GetCraftingAmount(id)
    local pl = LocalPlayer()
    if (not IsValid(pl) or not pl.CraftQueue) then return 0 end

    local total = 0
    for _, entry in ipairs(pl.CraftQueue) do
        if (entry.Item == id) then
...
local function ConstructRightPanel(panel, dontanimate)
    ...
    local function UpdateFavouriteButtonState()
        if (IsFavourite(SelectedItem)) then
            FavouriteButton:SetIcon("favorite.active")
    ...
    FavouriteButton.DoClick = function(self)
        gRust.PlaySound("ui.select")
        ToggleFavourite(SelectedItem)
        UpdateFavouriteButtonState()
        if (SelectedCategory == "Favourite") then
            gRust.Crafting.ItemGrid:Rebuild()
        end
    end
